...
export function WebsiteProfilePreview({
  ...
  const loadPreview = async () => {
    ...
    try {
      ...
      recordDirectoryObservabilityEvent("owner_website_preview_loaded");
      setPreview(body.preview);
      setSelected(new Set(
        (Object.keys(currentValues) as Array<keyof WebsiteProfilePreviewValues>)
          .filter((key) => hasValue(body.preview?.[key]) && !hasValue(currentValues[key])),
      ));
    } catch (cause) {
      setPreview(null);
  ...
  const available = preview
    ? (Object.entries({
      ...
      areaServed: preview.areaServed,
      accessibilityFeatures: preview.accessibilityFeatures,
    }) as Array<[keyof WebsiteProfilePreviewValues, string | string[] | null]>).filter(([, value]) => hasValue(value))
    : [];
  const labels: Record<keyof WebsiteProfilePreviewValues, string> = {
    ...
    summary: "Factual business summary",
    services: "Services and specialties",
    bookingUrl: "Booking link",
    menuUrl: "Menu or price-list link",
    areaServed: "Areas served",
    accessibilityFeatures: "Accessibility details",
  };
    ...
    <section className="mt-5 rounded-2xl border border-teal-900/10 bg-teal-50/70 p-4" aria-label="Owner-authorised website detail preview">
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-slate-600">With your permission, SuburbMates reads only machine-readable factual fields from this recorded website. It generates a new neutral summary from those facts; it never copies page text, images or HTML, and nothing is published automatically.</p>
        </div>
      </div>
      {!preview && (
        <label className="mt-4 flex cursor-pointer items-start gap-2 text-xs leading-5 text-slate-700">
          ...
          <span>I am authorised to ask SuburbMates to read structured public details from this website&apos;s homepage and up to four same-domain About, Services, Contact, Menu, Booking or Accessibility pages.</span>
        </label>
      )}
      ...
      {preview && (
        <div className="mt-4">
          {available.length === 0 ? <p className="rounded-xl border border-slate-200 bg-white p-3 text-xs leading-5 text-slate-600">No eligible structured details were found. You can still enter accurate details yourself.</p> : <><p className="text-xs leading-5 text-slate-600">Select the details you want added to the review form. Existing form values are never overwritten.</p><div className="mt-3 space-y-2">{available.map(([key, value]) => <label key={key} className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-800"><input type="checkbox" checked={selected.has(key)} disabled={hasValue(currentValues[key])} onChange={(event) => setSelected((previous) => { const next = new Set(previous); if (event.target.checked) next.add(key); else next.delete(key); return next; })} className="mt-1 size-4 rounded border-slate-400 text-teal-800 focus:ring-teal-800" /><span><strong>{labels[key]}</strong><br /><span className="break-words text-xs text-slate-600">{Array.isArray(value) ? value.join(", ") : value}</span>{hasValue(currentValues[key]) && <span className="mt-1 block text-xs text-slate-500">Already present in this form.</span>}</span></label>)}</div><button type="button" onClick={() => { const values = Object.fromEntries((Object.keys(currentValues) as Array<keyof WebsiteProfilePreviewValues>).map((key) => [key, selected.has(key) && !hasValue(currentValues[key]) ? preview[key] : currentValues[key]])) as WebsiteProfilePreviewValues; onApply(values); setApplied(true); recordDirectoryObservabilityEvent("owner_website_details_applied"); }} disabled={selected.size === 0} className="btn btn-primary mt-4 min-h-11 px-4 text-sm disabled:cursor-not-allowed disabled:opacity-60">Add selected details to form</button></>}
