...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = await request.json() as { runKey?: unknown; limit?: unknown }; const runKey = typeof body.runKey === "string" ? body.runKey : ""; const limit = typeof body.limit === "number" ? body.limit : 1;
    if (!/^[0-9a-f]{64}$/.test(runKey) || limit !== 1) return NextResponse.json({ error: "Invalid official-website enrichment request" }, { status: 400 });
