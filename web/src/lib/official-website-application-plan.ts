...
export type OfficialWebsiteApplicationPlan = { updates: Record<string, string | string[]>; facts: Array<{ fieldName: string; value: string; sourceUrl?: string; applied: boolean; conflict: boolean }>; appliedFields: string[]; conflictFields: string[] };
...
function sourceFor(facts: WebsiteFact[], fieldName: WebsiteFact["fieldName"], value: string) { return facts.find((fact) => fact.fieldName === fieldName && fact.value.trim() === value)?.sourceUrl; }
...
export function planOfficialWebsiteApplication(vendor: OfficialWebsiteApplicationVendor, facts: WebsiteFact[]): OfficialWebsiteApplicationPlan {
  ...
  if (summary) { const equal = same(vendor.description, summary); const apply = !vendor.description; evidence.push({ fieldName: "description", value: summary, sourceUrl: facts.find((fact) => fact.sourceUrl)?.sourceUrl, applied: apply, conflict: Boolean(vendor.description) && !equal }); if (apply) { updates.description = summary; appliedFields.push("description"); } else if (!equal) conflictFields.push("description"); }
