...
export function planOfficialWebsiteApplication(vendor: OfficialWebsiteApplicationVendor, facts: WebsiteFact[]): OfficialWebsiteApplicationPlan {
  ...
  for (const [factName, column] of scalar) for (const value of values(facts, factName)) { const current = vendor[column] as string | null; const equal = same(current, value); const apply = !current; const storageField = String(column); evidence.push({ fieldName: storageField, value, applied: apply, conflict: Boolean(current) && !equal }); if (apply) { updates[column] = value; if (!appliedFields.includes(storageField)) appliedFields.push(storageField); } else if (!equal && !conflictFields.includes(storageField)) conflictFields.push(storageField); }
