...
const SOURCE_CONTRACT_VERSION = "official-business-site-application-v2";
...
export async function runOfficialWebsiteEnrichment(runKey: string, requestedLimit: number) {
  ...
  try {
    for (const vendor of candidates) {
      const inspection = await inspectOfficialWebsite(vendor.website); const host = hostname(vendor.website); if (!host) throw new Error("An approved candidate has an invalid website host.");
      const { error: inspectionError } = await admin.from("official_website_inspections").insert({ vendor_id: vendor.id, source_key: SOURCE_KEY, source_contract_version: SOURCE_CONTRACT_VERSION, requested_url: vendor.website, resolved_url: inspection.sourceUrl, host_name: host, outcome: inspection.outcome, reason_code: reasonCode(inspection), robots_status: robotsStatus(inspection), terms_review_status: "approved", content_fingerprint: inspection.contentFingerprint, extracted_fact_count: inspection.facts.length, checked_at: inspection.checkedAt, freshness_due_at: freshnessDueAt(inspection.checkedAt), enrichment_run_id: created.id });
      ...
      const { error: auditError } = await admin.from("audit_events").insert({ actor_type: "service", action: "official_website_factual_enrichment", entity_type: "vendor", entity_id: vendor.id, reason: "Terms-approved, robots-aware structured factual inspection applied only empty unclaimed fields.", after_data: { applied_fields: plan.appliedFields, conflict_fields: plan.conflictFields, owner_control_preserved: true, media_or_page_copy_retained: false }, correlation_id: created.correlation_id }); if (auditError) throw new Error("Could not audit official-website factual enrichment.");
    }
