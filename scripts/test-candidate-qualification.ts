...
assert.deepEqual(
  qualifyCandidate(
    { ...valid, businessName: "Unrelated legacy name", streetAddress: "", phone: "", website: "", sourceUrl: "https://www.openstreetmap.org/node/123" },
    {
      ...options,
      existingListings: [{ id: "source-duplicate", businessName: "Different legacy name", sourceUrl: "https://www.openstreetmap.org/node/123/" }],
    },
  ).reasons,
  ["strong_duplicate"],
  "The same approved source record must not create a second listing when legacy fields are sparse.",
);
