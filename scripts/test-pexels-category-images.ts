...
assert.match(source, /local \$\{categorySlug\.replaceAll\("-", " "\)\}/);
assert.doesNotMatch(source, /"local service tools"/);
assert.match(source, /electrician at work/);
assert.match(source, /dog grooming salon/);
assert.match(source, /local fashion boutique/);
