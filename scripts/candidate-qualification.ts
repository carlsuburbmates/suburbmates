...
export function qualifyCandidate(
  ...
  options: {
    ...
    existingListings: readonly ExistingListing[];
  },
): Qualification {
  const normalized = {
  ...
  return {
    ...
    reasons,
    duplicateVendorId: duplicate?.id ?? null,
    normalized,
  };
}
...
function hasReachableContact(candidate: CandidateInput, normalized: Qualification["normalized"]) {
  ...
  return emailPattern.test(email) || normalized.phone.length >= 8 || Boolean(normalized.website);
}
...
function findNearDuplicate(normalized: Qualification["normalized"], listings: readonly ExistingListing[]) {
  return listings.some((listing) => {
    ...
    const address = normalizeText(listing.streetAddress);
    return (
      (normalized.businessName !== "" && normalized.businessName === name) ||
      (normalized.streetAddress !== "" && normalized.streetAddress === address)
    );
  });
}
...
export function normalizePhone(value: string | null | undefined) {
  return (value ?? "").replace(/\D/g, "").replace(/^61(?=\d{9,10}$)/, "0");
}
