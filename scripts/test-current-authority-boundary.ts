...
assert.match(automationReadme, /eleven active repository workflows/);
assert.doesNotMatch(automationReadme, /GitHub has seven active workflows/);
...
assert.match(automationWorkflows, /HubSpot Decision Inbox \| Hourly at minute 7/);
assert.doesNotMatch(automationWorkflows, /Every 15 minutes/);
assert.match(automationWorkflows, /Official website factual enrichment \| Daily/);
assert.match(automationWorkflows, /Licensed category-context images \| Daily/);
assert.match(automationWorkflows, /Operations health reconciliation \| Every six hours/);
