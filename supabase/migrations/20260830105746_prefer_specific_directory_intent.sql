-- D-018: Prefer the most specific grounded service phrase in a resident query.
--
-- The public reader remains a SECURITY INVOKER function over the existing safe
-- projection. It does not store a query, add a source, or change taxonomy.
-- When a phrase contains both a broad and specific recognised intent (for
-- example, "dog groomer" contains "dog"), the longest matching phrase wins.
