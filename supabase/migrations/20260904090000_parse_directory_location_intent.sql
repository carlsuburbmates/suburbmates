-- D-018: resolve a named public Darebin suburb from ordinary search phrasing
-- such as "cafe in Preston". This is grounded entirely in public.suburbs;
-- it does not retain query text or infer an address.
...
COMMENT ON FUNCTION public.search_published_vendors(TEXT, TEXT, TEXT, INTEGER, INTEGER) IS
  'Searches the safe public projection with grounded service and Darebin-location intent, literal matching and bounded typo tolerance. Query text is never retained.';
