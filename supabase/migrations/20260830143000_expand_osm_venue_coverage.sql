-- D-018: add only the new public categories required by the bounded OSM
-- expansion. These are source-tag mappings, not inferred reclassification of
-- existing listings.
INSERT INTO public.categories (slug, name, seo_description)
VALUES
  ('fitness', 'Fitness', 'Find local fitness businesses in Darebin.'),
  ('dance-studio', 'Dance Studios', 'Find local dance studios in Darebin.'),
  ('accommodation', 'Accommodation', 'Find local accommodation in Darebin.')
ON CONFLICT (slug) DO NOTHING;

-- Keep the same public-only reader and deterministic specificity rule. The
-- new aliases become useful only after a future qualified source observation;
-- this migration stores no query text and never reads private listing data.
...
AS $$
  ...
  intent_aliases(alias, target_category_slug) AS (
    VALUES
      -- Food, drink, hospitality and stays
      ...
      -- Health, beauty, fitness and pets
      ...
      -- Home, trade and vehicle services
  ...
  matched_intents AS (
    SELECT aliases.target_category_slug,
      cardinality(regexp_split_to_array(aliases.alias, '\\s+')) AS specificity
