-- D-018: broaden the bounded resident-language reader without retaining a
-- search query or importing any third-party vocabulary/data. Every target is
-- an existing public category; this only makes ordinary phrasing resolve to
-- the same safe public projection used by browse and literal search.
...
AS $$
  ...
  intent_aliases(alias, target_category_slug) AS (
    VALUES
      ...
      ('pet grooming', 'pet-grooming'), ('dog grooming', 'pet-grooming'), ('groomer', 'pet-grooming'), ('vet', 'veterinary'), ('vets', 'veterinary'), ('veterinarian', 'veterinary'), ('veterinary', 'veterinary'),
