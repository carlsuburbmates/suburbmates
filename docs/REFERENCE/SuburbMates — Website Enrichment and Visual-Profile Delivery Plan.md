...
2. Implement and test the factual-pilot source contract, schema/RLS protections, bounded crawler/parser, evidence/conflict lifecycle, terms-aware weekly runner and quiet `/ops/System` health reporting. **Version 3 implemented locally:** the runner evaluates robots and clearly linked same-domain terms automatically, retains no page or terms content, holds possible restrictions, honours operator overrides, and skips still-fresh inspections so successive bounded runs advance through eligible listings.
