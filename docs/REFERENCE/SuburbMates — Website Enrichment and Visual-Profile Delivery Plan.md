...
**Status:** in delivery. The factual-pilot source contract, private inspection ledger, bounded parser and daily authenticated application runner are live. Version 3 replaces the manual-every-domain choke point with deterministic robots and clearly linked same-domain terms assessment: possible restrictions remain held for operator review, explicit operator blocks always win, and current inspections are skipped until their 31-day freshness window expires so bounded batches advance through the cohort. It can fill only empty factual fields on unclaimed published profiles. JSON-LD is supplemented by explicit contact/action links and schema microdata; service-page headings remain evidence-only during their quality rollout. The owner-rich factual import and review-first profile fields are in this release. Pexels-backed category imagery is configured and operating through the server-only provider contract; live public acceptance remains part of the delivery evidence.
...
Before any production writes, inspect a deterministic 25–50-domain sample for domain match, robots availability, accessible structured data, terms signals and extraction quality. The production runner repeats those controls per domain: a clearly linked same-domain terms page is read transiently and fingerprinted, possible automated-access restrictions or unavailable linked terms are held, and operator overrides remain available. Absence of a discovered restriction permits only structured public business facts; it is never permission to copy protected material. A safe pilot cohort must be broad across categories and suburbs, not cherry-picked for easy sites.

### A2. Bounded extraction

For a passing domain, request the homepage and only labelled same-domain Services, Contact, Booking and Menu pages. Prefer JSON-LD, then explicit `tel:`/`mailto:` contact links, exact labelled booking/menu destinations and schema.org microdata. Candidate service headings from a labelled Services page start as low-confidence evidence only and require production quality evidence before promotion. Do not crawl site-wide, follow arbitrary links, infer a service from prose, retain cookies, submit forms, enter logins, access customer content, bypass bot protection or store raw pages beyond the minimum evidence fingerprint needed for a refresh decision.
...
Each business-profile use displays **“Representative image for this category”**, explains that the licensed context is not supplied by or specific to the business, and includes the provider and photographer credit plus provider link. Retain provider photo ID, original URL, photographer, licence snapshot, keyword map version, selection date and crop metadata. A stock image never counts as an owner-approved image or as a monetisation quality-gate success.
...
2. Implement and test the factual-pilot source contract, schema/RLS protections, bounded crawler/parser, evidence/conflict lifecycle, terms-aware weekly runner and quiet `/ops/System` health reporting. **Version 3 live verified:** the runner evaluates robots and clearly linked same-domain terms automatically, retains no page or terms content, holds possible restrictions, honours operator overrides, and skips still-fresh inspections from completed runs so successive bounded runs advance while failed applications remain retryable. The workflow uses 25 independent one-site idempotent requests so a Worker cutoff cannot leave a whole batch falsely green. Production run `33970075850` completed all 25 batches and safely applied 16 empty factual fields across four unclaimed profiles while retaining three conflicts privately.
...
4. Implement the owner-authorised rich-import review, exact-image permission attestation and existing-media-workflow integration. **Implemented; genuine-owner acceptance pending:** the owner preview reads bounded same-domain structured pages and can add a neutral fact-derived summary, services, booking/menu links, areas served and accessibility details to the existing immutable-audit review request. An authenticated owner may explicitly request a private, non-persistent preview of up to six same-domain image candidates from the recorded website, choose or manually enter one, edit its type and alt text, and attest the business's reuse rights. The selected JPEG, PNG or WebP is retrieved through the existing two-megabyte, same-domain redirect boundary and copied to private proposal storage; existing moderation and immutable audit remain the sole publication path. No candidate preview is permission, publication or evidence of ownership.
