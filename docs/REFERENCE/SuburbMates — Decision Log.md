...
- **Date:** 4 September 2026
- **Owner decision:** The current owner-initiated, one-homepage JSON-LD preview is a compliance foundation, not the finished enrichment model. SuburbMates must automatically refresh every eligible published profile that already records an official HTTPS website, so the free directory can demonstrate real resident and owner value before any payment capability is considered.
- **Website-source rule:** The lifecycle is a versioned `official_business_site` source contract tied to the listing's recorded canonical domain. It must make bounded, polite requests; retain a source URL, observation time, freshness, content fingerprint and field-level evidence; honour a blocked/unsupported response as a quiet system outcome; and never use a website to discover or publish a new business. It may inspect a bounded homepage plus a small, same-domain set of clearly labelled service, menu, booking or contact pages. It must never collect login-only content, customer content, analytics, cookies, form submissions, arbitrary linked pages or site-wide copies.
- **Factual-content rule:** Automation may extract explicit business facts such as structured hours, services/specialties, booking and menu destinations, contact channels and narrow useful attributes. It must create a new, concise factual profile summary from retained structured facts rather than copying page prose or presenting an AI-written claim. The summary and each extracted fact remain source-backed, freshness-limited and conflict-aware. Existing owner-confirmed or non-empty public values are never overwritten automatically; ambiguity remains private evidence.
- **Media and owner rule:** Website images, logos and third-party media remain excluded from automatic collection and display. Separately licensed stock imagery may provide an automatically selected, category-scoped visual context only when its provider API terms, attribution and usage limits are recorded in a versioned source contract. It must be horizontal, all-ages and free of identifiable people, brands and logos; it must state **“Illustrative [category] image — not a photo of this business”** alongside the provider/photographer credit; and it may never be represented as the business, its premises, staff, product or endorsement. The owner may replace it, upload their own media, or explicitly authorise a website image through the existing moderated workflow. A website image file is neither fetched, stored nor shown publicly until that owner permission exists. This decision enables no unsolicited marketing email.
- **Presentation and measurement rule:** Homepage and category-page discovery modules may prefer profiles with more source-backed useful facts, while organic directory search remains neutral and unchanged. Aggregate first-party measurements may compare profile views and direct contact, booking, menu and directions actions for enriched versus unenriched profiles without retaining visitor identities or search terms.
- **Commercial rule:** Stripe, price, entitlements, paid placement and payment implementation are paused. They remain disabled until this enrichment lifecycle is deployed, its accuracy/freshness and owner controls are verified, and aggregate contact/booking outcomes demonstrate useful free-directory value.
- **Evidence to close:** Source-contract and schema/RLS review; bounded crawler, parser, evidence/conflict and owner-media-approval tests; controlled acceptance; deployment and in-app production verification; and a documented aggregate outcome observation window. No synthetic production business, owner, content or media record may be created merely to close the evidence.
