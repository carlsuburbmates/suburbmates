...
- **Claimed-pilot access amendment — 6 September 2026:** A business owner or authorised representative may create a basic authenticated account through a hostname-restricted, Turnstile-verified one-time email code. Authentication only identifies the email account: it never assigns a listing, grants ownership, publishes or edits a profile, approves evidence, enables payment or bypasses Ops. An existing listing still requires the exact-email claim RPC or private claim-help evidence and an operator decision; missing-business submissions remain private and review-first. Existing-account password and email-code login remain unchanged. This narrow access route supersedes the former UI-only rule that all accounts must be manually pre-provisioned, because that rule prevented a genuine 25–50 business pilot while Supabase project signup was already enabled at the backend.
- **Atomic application and rollback amendment — 7 September 2026:** A public empty-field application from the official-website runner must commit its field evidence, private conflicts, listing mutation and immutable audit event in one database transaction while holding the run and listing locks. Any ownership change, intervening populated value, unsupported field or invalid evidence rejects the whole application. A protected operator may reverse a specific application only while the listing remains unclaimed and every value applied by that run still exactly matches the audited state. The rollback restores only those fields, retains the source evidence as superseded and appends a reasoned audit event; it must fail closed rather than erase later owner or operator work.
- **Linked factual-page promotion amendment — 7 September 2026:** The temporary evidence-only restriction on facts from already-authorised linked factual pages is retired. After the recorded homepage passes official-business identity, robots and linked-terms gates, the bounded runner may apply explicit structured facts from up to four robots-allowed same-domain Services, Contact, Booking, Menu or Accessibility pages under the same empty-field-only, unclaimed-listing, atomic evidence/conflict/audit and guarded-rollback rules. Each value retains its exact page URL and lower confidence than a homepage fact. Page prose, images, reviews, testimonials and marketing descriptions remain excluded; an existing value remains unchanged and differing evidence remains private conflict evidence.
- **Scalar ambiguity and useful-summary amendment — 7 September 2026:** A phone, email, address, schedule, booking link or menu link may fill an empty public field only when the bounded inspection produces exactly one eligible value for that field. Multiple branch or location values remain private conflict evidence instead of whichever value happened to be processed last. Obvious placeholder phones are rejected before evidence retention. A generated description must contain at least one explicit service or service-area fact; hours alone may supplement a useful summary but may not become the entire description. Production acceptance reverses only exact unchanged values that fail these rules, retains their source history as rejected or superseded and appends a correction audit.
- **Explicit factual-element rollout amendment — 7 September 2026:** The implemented parser may supplement JSON-LD with explicit `tel:` and `mailto:` contact links, exact labelled booking/menu destinations and schema.org `itemprop` microdata found on the already-authorised bounded pages. These facts retain the exact page URL and pass the same identity, robots, terms, scalar-uniqueness, empty-field, conflict, atomic audit and rollback gates as JSON-LD. Equivalent Australian phone formats are one fact, not a false conflict. Obvious template emails/phones are rejected. Candidate service headings from a labelled Services page are retained initially as lower-confidence evidence only; questions, FAQ/feedback headings and generic marketing headings are rejected, and no heading can populate services or a description until production evidence supports a separate promotion decision. Arbitrary prose, testimonials, reviews and marketing copy remain excluded.
