...
| Modern directory direction | The current production pipeline accepts only `openstreetmap-candidate-v1` and treats missing reachable contact as a qualification exception. Public profiles remain sparse and media-free. | Implement D-018's approved-source registry, field-evidence lifecycle, safe enrichment/refresh, intent-aware search and rich public profile experience; retain the existing model only as a controlled migration baseline. |
