...
- **Explicit factual-element rollout amendment — 7 September 2026:** The implemented parser may supplement JSON-LD with explicit `tel:` and `mailto:` contact links, exact labelled booking/menu destinations and schema.org `itemprop` microdata found on the already-authorised bounded pages. These facts retain the exact page URL and pass the same identity, robots, terms, scalar-uniqueness, empty-field, conflict, atomic audit and rollback gates as JSON-LD. Equivalent Australian phone formats are one fact, not a false conflict. Candidate service headings from a labelled Services page are retained initially as lower-confidence evidence only; they cannot populate services or a description until production evidence supports a separate promotion decision. Arbitrary prose, testimonials, reviews and marketing copy remain excluded.
