...
- The terminal source-health timestamp correction is **deployed—verification pending** at commit `df2d839` on Cloudflare Worker `37c84f27-3c05-4cfe-9acd-e4639f14fe17`. The terminal source-run callback now writes the same current timestamp to `integration_health.updated_at` as it writes to `last_success_at` or `last_failure_at`; previously the System detail could retain the earlier “started” time after a completed run. The served authenticated `/ops/System` page remains **All clear**. The exact new timestamp is intentionally pending the next genuine source callback: no synthetic production callback or audit event was created merely to demonstrate it.
