...
- the current Cloudflare Worker deployment is version `164799ca-d8ae-49bd-875a-6f4ba39b3468` from commits `d8334f3`, `91b8ba5` and `88212d1` (following the `d7badbb` hospitality-source enrichment). It serves the fail-closed source-registry guard and its source-specific `/ops/System` explanation, and the unauthenticated candidate endpoint still returns `401` without reading or changing any candidate data;
