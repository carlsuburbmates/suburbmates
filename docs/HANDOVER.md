...
- The owner-media proposal flow is **live verified** at `6412e19`: a claimed owner sees a private local preview before submission, can insert an editable permission statement with one click, and sees the private pending-review state without a manual refresh. The existing private upload storage, claimed-owner authorisation, moderation, audit basis and public-only-after-approval rule did not change. Current production aggregate: **0** owner-media proposals and **0** approved owner-media items. No synthetic upload was created to test it.
