...
- the current Cloudflare Worker deployment is version `cbee69b8-031f-41bb-960d-ebe2158d6547` (deployment 150). It serves the fail-closed source-registry guard and its source-specific `/ops/System` explanation, and the unauthenticated candidate endpoint still returns `401` without reading or changing any candidate data;
