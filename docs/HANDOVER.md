...
- the current Cloudflare Worker deployment is `a4166e23-d264-43b0-957b-c81cb8cd7085` from commit `a1fa153`; it serves the fail-closed source-registry guard and its source-specific `/ops/System` explanation, and the unauthenticated candidate endpoint still returns `401` without reading or changing any candidate data;
