...
- the current Cloudflare Worker deployment is version `66b23ea2-b851-459c-b7f7-f7491c97710e` from commit `f200184` (following the `d7badbb` hospitality-source enrichment). It serves the fail-closed source-registry guard and its source-specific `/ops/System` explanation, and the unauthenticated candidate endpoint still returns `401` without reading or changing any candidate data;
