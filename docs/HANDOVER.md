...
### Latest verified live state — 31 August 2026 (Australia/Melbourne)
...
- Two interrupted pre-recovery Tax Practitioners Board receipts were closed through the deployed server-authorised stale-run finaliser as failed, audit-preserving records; neither changed a listing. The final source run and `tax_practitioners_board_source` health are healthy, with no active processing receipt. The Worker is now `84f0a65e-228f-42f9-b092-be2e09a7f6ef` (including normalised source whitespace, bounded handoff recovery, and a one-time retry for transient public directory reads). The initial post-source browser read briefly rendered the existing directory-error boundary; the served final worker then passed browser acceptance. Persistent public-read failures still render that explicit error rather than silently empty results.
