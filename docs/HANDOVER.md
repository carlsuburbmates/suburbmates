...
- The official Victorian locality-boundary enhancement is **deployed—verification pending** at commit `6c303ae`, incorporated in current Cloudflare Worker `1fecb94c-36b6-4695-a0c1-880067fd99ab`. An OSM feature without a supported explicit locality can now receive only a containing, existing Darebin locality from the CC BY 4.0 Victorian boundary reference; its original OSM locality always wins. The boundary source cannot create a business, address, contact fact, ownership, or publication change. Every applied locality retains its own `geoscape_vic_localities` field-evidence row. The normal [production OSM refresh 33463175979](https://github.com/carlsuburbmates/suburbmates/actions/runs/33463175979) is still running its earlier singleton handoff runner: at the last checked live snapshot, one of its eight handoff shards had completed successfully and **122** listings held applied boundary locality evidence. Do not call that terminal outcome or source health final until the workflow's own callback completes.
