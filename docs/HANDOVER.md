...
- the current Cloudflare Worker deployment is version `044804a6-d8b1-4775-841b-2dd0fbc9cc82` from commit `d7badbb`. It serves the fail-closed source-registry guard and its source-specific `/ops/System` explanation, and the unauthenticated candidate endpoint still returns `401` without reading or changing any candidate data;
