...
- Source-health freshness is **deployed—verification pending** at commit `20f7fcc` on Cloudflare Worker `0c679cec-040f-4bc7-be0c-7ba3e2456e71`; the clean [Verify run 33453348251](https://github.com/carlsuburbmates/suburbmates/actions/runs/33453348251) is green. All three approved source workflows now share the same run-level lifecycle: acquire and retain the source artifact, mark the protected handoff running, complete the paced private handoff, then record one terminal outcome. The callback and direct candidate/contract-hold paths write the current `integration_health.updated_at` value with their success or failure state, so System cannot present an earlier started time as the later check. Authenticated in-app `/ops/System` remains **All clear**. The existing Victorian and Tax Practitioners Board timestamps remain historical until their next genuine monthly handoffs; no synthetic production callback, source run, listing change or fabricated audit event was created to demonstrate them.
...
- Public-profile coverage is **live verified** at commit `1d82e16` on Cloudflare Worker `d297aef1-082f-4abd-bc18-ff25006bafa4`. Authenticated in-app-browser acceptance of `/ops/System` showed the quiet aggregate baseline: **2,259** published profiles; **1,599** with a street address; **827** with at least one direct contact route; **223** with a description; and **231** with reported hours. The new System card reads counts only from `published_vendors`; it does not retrieve private requests, create operator work, store a coverage snapshot, change a listing, or make a quality/currentness claim. This is the current authoritative measure of the profile-enrichment gap.
