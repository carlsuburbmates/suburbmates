...
- the current Cloudflare Worker deployment is version `5cc9cd6c-5971-4557-9cbd-16a2f4103779`, including commit `cc82d93` (safe public source summaries). It retains the fail-closed source-registry guard and source-specific `/ops/System` explanation, and the unauthenticated candidate endpoint still returns `401` without reading or changing any candidate data;
...
- Current Worker deployment: Cloudflare version `5cc9cd6c-5971-4557-9cbd-16a2f4103779`, including `cc82d93`; the in-app browser confirmed the served browse page's service-first search, the authenticated owner dashboard's review-only opening-hours field, and a live source-backed profile's safe Information sources card on 30 August 2026. No schedule, image or other owner change was submitted during acceptance. This is not evidence that every public template or mobile viewport has been accepted.
