...
4. **Darebin Council and Darebin-linked business associations** are the next coverage priority. A source may be automated only after the provider grants a reusable feed or written permission that covers persistent public display, refreshes and attribution, and the feed supplies a stable business identity plus a non-postal Darebin location.
...
6. **Australian Business Register (ABR) bulk extract** is CC BY 3.0 AU and may be used later for aggregate coverage measurement. It is not a current public-listing feed: its location is only state/postcode-level, it has no useful category or direct-contact fields, and its legal-name fields can identify an individual. A future measurement job must discard legal names and never create, enrich or publish a listing from this extract alone.
7. **A separately licensed business dataset** may become a coverage source only when its agreement expressly permits persistent directory display, refreshes, owner-claim workflows and attribution.
