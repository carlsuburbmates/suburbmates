...
6. **Business-owned websites** are never a business-discovery source or substitute for a licensed source feed. D-020 remains the owner-only private JSON-LD preview. Under D-021, SuburbMates may separately run a small, evidence-limited factual-enrichment pilot against 25–50 already-published listings with a recorded verified official HTTPS domain, only after a versioned source contract, robots/terms review and controlled acceptance. It must fail closed on disallow, robots retrieval failure, uncertain identity, authentication, paywall or bot challenge; collect only bounded factual material; retain provenance/freshness/conflicts; and never copy website prose, reviews, testimonials, logos or images. Claimed listings may instead use the explicit owner-authorised rich-import path. Neither path creates a new business from a website.
...
## Candidate-source assessment — 4 September 2026

- **Darebin food-business pages:** Council confirms that food businesses are registered or notified under the Food Act, and its privacy policy confirms the Health Protection Unit maintains that register. The public material is regulatory guidance, however: it provides no reusable premises feed, compatible public-display licence, stable record ID or refresh contract. The existence of the register is not permission to copy it, so it is not an automated catalogue contract. A provider-approved feed or written agreement remains required.
