...
6. **Business-owned websites** are not a bulk automated enrichment source. After an approved owner explicitly opts in, SuburbMates may privately preview bounded machine-readable JSON-LD from that listing's already-recorded website and place selected contact, canonical social-link or hours values into the existing moderated profile-change form. It never crawls unclaimed sites, linked pages or images; never stores page copy/HTML; and never updates a public profile automatically. This owner-authorised form aid is not a versioned discovery contract or substitute for a licensed source feed.
