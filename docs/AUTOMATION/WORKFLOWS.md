...
GitHub enrichment and visual context (active on main)
  -> bounded official-website factual enrichment with robots/terms gates
  -> licensed Pexels category-context selection with credit and freshness
  -> existing public values remain unchanged on ambiguity or failure

GitHub Operations health reconciliation (active on main)
  -> monitor scheduled workflow failure, cancellation, and staleness
  -> maintain one stable plain-language recovery issue
  -> close it automatically after recovery
...
| HubSpot Decision Inbox | Hourly at minute 7; manual dispatch | Call the protected reconciliation endpoint; create/update one low-detail Task for genuine Ops work and close tasks whose linked work is no longer actionable | GitHub Actions, Cloudflare, HubSpot Task API | Short decision inbox only | **Active on `main`; current hourly scheduled runs pass** | Task-only one-way mirror. No HubSpot contact, company, deal, marketing, billing, directory or private-evidence sync; it cannot change SuburbMates data. |
| Official website factual enrichment | Daily at `20:13` UTC; manual dispatch | Advance one 25-site bounded cohort; enforce official-domain identity, robots/terms, freshness, evidence, scalar uniqueness, atomic empty-field application and private conflicts | GitHub Actions, Cloudflare, Supabase, official business websites | Private run/evidence/conflict records and narrowly eligible public empty fields | **Active on `main`; accepted production cohorts recorded in `SUB-31`** | Never discovers or publishes a business, copies prose/media, overwrites an existing or owner-controlled value, or applies ambiguous evidence |
| Licensed category-context images | Daily at `20:23` UTC; manual dispatch | Refresh a bounded stale/missing category set through the server-only Pexels contract; apply suitability, credit, diversity and freshness controls | GitHub Actions, Cloudflare, Supabase, Pexels API | Credited category-context image assignments | **Active on `main`; production selection and public presentation verified** | Context only; never business evidence, endorsement, ranking or a claim that the image depicts the listed business |
| Operations health reconciliation | Every six hours at minute 37; after monitored workflow completion; manual dispatch | Inspect the latest completed monitored runs; detect failure, cancellation and staleness; update one stable plain-language GitHub issue and close it after recovery | GitHub Actions, GitHub API and Issues | One consolidated recovery record | **Active on `main`; failure and recovery behavior verified** | No Supabase, listing, owner, request, publication or audit mutation |
...
Hourly, the workflow asks the protected production endpoint to reconcile only genuine pending Ops decisions. HubSpot receives a plain task title, priority and protected SuburbMates link; it never receives contact text, requester or claimant details, ABNs, evidence, media paths, audit notes, provider errors or directory-wide records. The workflow uses HubSpot Tasks only. A HubSpot failure does not block the Ops action; the next reconciliation closes a task missed during an outage.

### 6. Internal health and Ops display

Source: `supabase/migrations/20260715173000_internal_health_monitoring.sql`.

The scheduler calls `refresh_internal_operations_health()` hourly. It reports the state held in database tables; it does not observe GitHub Action runs or GitHub artifacts. The separate Operations health workflow observes those scheduled runs and maintains the stable GitHub recovery issue linked from `/ops/System`; it does not copy raw workflow data into Supabase. `/ops/system` presents the database health rows, jobs, audit events and safe recovery link to an authenticated active operator. These complementary monitors must never affect listings, ownership, claims, payments, or publication.
