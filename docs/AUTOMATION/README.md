...
- GitHub has eleven active repository workflows on `main`: Verify; four approved-source catalogue discovery workflows; Website safety evidence; Production smoke; HubSpot Decision Inbox; licensed category-context images; official-website factual enrichment; and Operations health reconciliation. The HubSpot workflow is an hourly one-way Ops companion: it creates or closes low-detail Tasks only and does not change SuburbMates data.
...
- The database health monitor does not ingest GitHub workflow runs or artefacts. Its green automation-queue status means only that the local job table has no failed or overdue rows. The separate GitHub Operations health reconciliation checks scheduled-run failure, cancellation and staleness and maintains one stable recovery issue; neither signal substitutes for the other.
...
2. **Keep GitHub evidence distinct from the database health badge.** The consolidated Operations health workflow monitors GitHub scheduled-run conclusions and cadence without copying raw run data into Supabase. The candidate handoff records its own database run and job state, but a database health row alone is still not proof that every GitHub workflow ran.
