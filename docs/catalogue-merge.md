...
This document outlines the local staging merge tool, which combines the manually curated primary source (`data/vendor-candidates.csv`) with the staged OpenStreetMap acquisition source (`data/vendor-candidates-osm.csv`). It creates a comparison CSV only; it does not import, publish, or make a live duplicate decision.
...
- **Staging Merge Rule**: A record is merged into the comparison CSV when its normalized business name matches exactly, AND:
...
> **Important boundary:** this broad staging merge rule is not the live duplicate policy. The persistent OpenStreetMap handoff uses the approved qualification path, and Ops follows Decision Log D-014: address-only similarity is never duplicate evidence. Do not use this generated CSV to merge, remove, or decide live listings.
